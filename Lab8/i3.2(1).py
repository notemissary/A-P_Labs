...
T = [0] * len(W)  # Генерируем таблицу в длину подстроки.
...
while m+i < len(S):  # Если сумма этих двух внезапно превысит размер S, то
                     # все потеряно.
    if W[i] == S[m+i]:  # Тут мы просто сравниваем символы W и S.
        if i == len(W)-1:  # Если i примет значение длины W - 1, то это
                           # успех!
        ...
        if T[i] > -1:  # Если у нас i элемент в Т больше -1, то значит, что
                       # где-то в проверке было начало подстроки
